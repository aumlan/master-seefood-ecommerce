...
@section('content')
    <div class="row">
        <div class="col-xl-4 col-lg-4">
            <div class="card">
                <div class="card-body">
                    <form class="form form-vertical" method="POST" action="{{route('admin.currency.currency_update')}}" enctype="multipart/form-data">
                        @csrf
                        @include('Backend.currency.part.form',['currencys'=>$currencys,'btnText'=>'Update'])
                    </form>
                </div>
            </div>
        </div>
        <div class="col-xl-8 col-lg-8">
            <div class="card">
                <div class="card-body">
                    @include('Backend.currency.part.currency_list')
                </div>
            </div>
        </div>
    </div>
...
<script>
    function deletecurrency(id){
      var $currency =  confirm('are you Sure You Want to Delete this currency');
      if($currency){
        $.ajax({
