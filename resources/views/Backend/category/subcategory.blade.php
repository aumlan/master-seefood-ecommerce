...
@section('content')
    <div class="row">
        <div class="col-xl-4 col-lg-4">
            <div class="card">
                <div class="card-body">
                    <form class="form form-vertical" method="POST" action="{{route('admin.categories.subcategory.store')}}" enctype="multipart/form-data">
                      @csrf
                        @include('Backend.category.part.subcategory_form')
                    </form>
                </div>
            </div>
        </div>
        <div class="col-xl-8 col-lg-8">
            <div class="card">
                <div class="card-body">
                    @include('Backend.category.part.sub_category_list')
                </div>
            </div>
        </div>
    </div>
...
<script>
    function deleteCategory(id){
      var category =  confirm('are you Sure You Want to Delete this Sub Category');
      if(category){
        $.ajax({
            url: "/admin/category/subcategory/delete/"+id,
            type: 'GET',
